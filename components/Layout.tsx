    ...
    <div className="layout-color relative min-h-screen">
      ...
      <header className="bg-red-plus sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 py-3 flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="font-bold text-white text-slate-800 tracking-tight leading-none">Comunidad MO</h1>
            <div className="flex items-center gap-1 mt-0.5">
              <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></div>
              <span className="text-[10px] text-white font-medium uppercase tracking-wider">Cloud Sync On</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button className="p-2 text-white hover:bg-red-500 rounded-full transition-colors relative">
            <CloudCheck size={20} />
          </button>
          <button className="p-2 text-white hover:bg-red-500 rounded-full transition-colors relative">
            <Bell size={20} />
...
const NavButton: React.FC<NavButtonProps> = ({ active, icon, label, onClick }) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center gap-1 transition-all duration-300 ${active ? 'text-blue-600 scale-105' : 'text-slate-400 hover:text-slate-600'}`}
  >
    {icon}
