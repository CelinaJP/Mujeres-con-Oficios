...
const DirectoryView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h2 className="flex items-center justify-center text-3xl font-bold text-slate-800">Directorio</h2>
        <p className="flex items-center justify-center text-slate-500">Encontrá profesionales de confianza.</p>
      </header>

      {/* Search & Filters */}
      <div className="space-y-3">
        <div className="relative border border-slate-400 rounded-2xl">
          ...
          {['Todas', 'CABA', 'Norte', 'Sur'].map(zone => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all border ${
                filterZone === zone
                ? 'bg-blue-plus border-bg-blue-plus text-white shadow-md'
                : 'bg-white border-slate-200 text-slate-500 hover:border-bg-blue-plus'
              }`}
            >
              {zone}
            </button>
      ...
      <div className="space-y-4 !mb-20">
        {filtered.map(pro => (
          <div key={pro.id} className="bg-white rounded-3xl p-5 shadow-sm border border-slate-400 overflow-hidden">
            <div className="flex items-start justify-between mb-4">
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-blue-plus text-sm font-semibold mt-0.5">{pro.specialty}</p>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {pro.coursesCompleted.map((course, idx) => (
                <span key={idx} className="flex items-center gap-1 bg-violet-100 text-blue-plus px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider">
                  <Award size={10} />
                  {course}
                </span>
              ))}
            ...
            {/* Mini Portfolio */}
            <div className="grid grid-cols-3 gap-2 mb-5 max-w-3xl">
